    ...
    if total_cascades != 0:
        ...
        for edge in g.es[edges]:
            ...
            if float(bernouli_weight) > 0.000001:
                bernouli_graph.write(str(g.vs[edge.tuple[1]]["name"]) + " " + str(
                    g.vs[edge.tuple[0]]["name"]) + " " + bernouli_weight + "\n")
                # ------- Avg Dt / delta
            if edge["Dt"] > 0:
                time_weight = str("%.6f" % (np.exp(-(float(edge["Dt"]) / edge["Inf"]) / delta)))
                time_graph.write(str(g.vs[edge.tuple[1]]["name"]) + " " + str(
                    g.vs[edge.tuple[0]]["name"]) + " " + time_weight + "\n")
...
for e1 in g1.es:
    try:
        ...
        f.write(str(g1.vs[e1.tuple[0]]["name"]) + " " + str(g1.vs[e1.tuple[1]]["name"]) + " " + str(
            e2["weight"] * e1["weight"]) + "\n")
    except:
        continue
f.close()
