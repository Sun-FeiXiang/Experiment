...
def CBPCA(G, k):
    ...
    NI = dict()
    for u in G.nodes:
        NI[u] = math.sqrt(node_degree[u] ** 2+ node_h[u] ** 2) * node_E[u]# + node_h[u] ** 2
    S, timelapse = [], []
    Visited = dict()  # 节点访问标志
    for node in G.nodes:
        Visited[node] = False
    ...
    while i < k:
        ...
        while NI_i < len(NI) and max_Influence < node_degree[NI[NI_i][0]]:  # 截至条件
            ...
            NI_i = NI_i + 1
        print("本轮遍历次数：", NI_i,"，选取",max_Influence_Node,"为种子，未覆盖邻居个数：",max_Influence)
...
if __name__ == "__main__":
    ...
    df_IC_hep.to_csv('../../data/output/test/IC_method1(p=0.01,I=1000)_hep_Graph.csv')
    print('文件输出完毕——结束')
